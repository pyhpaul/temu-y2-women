...
The system SHALL retrieve `dress` candidates from local MVP evidence files and apply supported filters before composition.

#### Scenario: Retrieve eligible candidates from local evidence
- **WHEN** the normalized request is valid and active evidence records exist in the local MVP files
...
The system SHALL provide fixed validation scenarios that exercise both successful and failing requests for the MVP flow.

#### Scenario: Validate a successful summer vacation request
- **WHEN** the MVP validation suite runs against a predefined summer vacation `dress` request
- **THEN** it verifies that the flow produces a normalized request, selected strategy, composed concept, and prompt bundle without error

#### Scenario: Validate a constrained failure path
- **WHEN** the MVP validation suite runs against a predefined request whose constraints eliminate all valid outcomes

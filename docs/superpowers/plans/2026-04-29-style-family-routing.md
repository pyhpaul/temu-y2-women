...
- [ ] 先写 loader/selector 失败测试，固定 4 个 family 的 config shape
- [ ] 运行 `python -m unittest tests.test_style_family_repository tests.test_style_family_selector -v`，确认先失败
- [ ] 实现 config loader、active value 校验、显式选择和自动 fallback heuristic
- [ ] 重新运行上述测试直到通过
...
- [ ] 先写失败测试，固定 `style_family` 为 optional request field
- [ ] 覆盖 unknown family / empty string / explicit family success 路径
- [ ] 修改 dataclass 与 request normalize
- [ ] 在 success result 中输出 `selected_style_family`
- [ ] 跑 `python -m unittest tests.test_request_normalizer tests.test_orchestrator -v`
...
- [ ] 先补足 family 所需新 tags / value
- [ ] 新增能拉开差异的 fabric / neckline / sleeve / color / detail / length 元素
- [ ] 确保新 active values 与 taxonomy 完全对齐
- [ ] 跑 `python -m unittest tests.test_evidence_repository -v`
...
- [ ] 先写失败测试，覆盖 hard filter / blocked value / soft boost / explicit conflict fail-closed
- [ ] 运行对应测试，确认当前失败
- [ ] 实现 family-aware candidate filtering/scoring
- [ ] 保证 strategy 与 style family 可以叠加
- [ ] 重新运行相关测试直到通过
...
- [ ] 先写失败测试，固定 `selected_style_family_id` 出现在 result / factory spec
- [ ] 实现 orchestrator 的 family 加载与传递
- [ ] 实现 factory spec 的 known/inferred family 补充
- [ ] 运行 `python -m unittest tests.test_factory_spec_builder tests.test_orchestrator -v`
...
- [ ] 先写失败测试，固定 4 个 family 的主图 prompt shell 差异
- [ ] 要求至少在 subject / scene / lighting / styling / avoid 项上可见分化
- [ ] 实现 family-specific prompt shell
- [ ] 运行 `python -m unittest tests.test_prompt_renderer -v`
...
- [ ] 运行 `python -m py_compile temu_y2_women\\style_family_repository.py temu_y2_women\\style_family_selector.py temu_y2_women\\models.py temu_y2_women\\request_normalizer.py temu_y2_women\\evidence_repository.py temu_y2_women\\orchestrator.py temu_y2_women\\prompt_renderer.py temu_y2_women\\result_packager.py temu_y2_women\\factory_spec_builder.py`
- [ ] 运行 `python C:\\Users\\lxy\\.codex\\rules\\hooks\\validate_python_function_length.py .`
- [ ] 运行 `python C:\\Users\\lxy\\.codex\\rules\\hooks\\validate_forbidden_patterns.py .`

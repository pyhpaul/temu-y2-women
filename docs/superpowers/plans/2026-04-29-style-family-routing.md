...
> Completion status: implementation tasks 1-8 are complete on `main` as of 2026-05-01. Tasks 1-7 were verified with focused style-family suites, style modules `py_compile`, function-length guard on `temu_y2_women tests`, and forbidden-pattern guard. Task 8 local artifacts are under `tmp/style-family-routing/`: requests, concepts, hero-front prompts, render reports, four hero-front images, manifest, and visual acceptance notes. callxyq remained blocked with `/images/generations` 500 `task_failed`, but the latest API key/base URL from the provided `fashion-visual-lock-pipeline` `.env` rendered all four images successfully via `http://192.168.5.117:4001`.
...
- [x] 准备 4 个请求：
  ...
  - `city-polished`
  - `party-fitted`
- [x] 只跑 `hero_front generate`
- [x] 落盘 prompt / concept / render report / manifest
- [x] 落盘 image
- [x] 验收“只看图是否一眼能区分”

...
- [ ] 写默认 5 检查报告结构测试
- [ ] 写 `--check` 过滤测试
- [ ] 写 timeout / 502 / 503 结构化映射测试
- [ ] 运行 `python -m unittest tests.test_image_gateway_smoke -v`，确认先失败
...
- [ ] 实现原始 JSON / multipart 请求能力
- [ ] 实现 1x1 PNG 内置参考图
- [ ] 实现默认 5 检查与 JSON 报告聚合
- [ ] 重新运行 `python -m unittest tests.test_image_gateway_smoke -v`
...
- [ ] 写 CLI 参数转发与配置解析测试
- [ ] 写配置错误路径测试
- [ ] 运行 `python -m unittest tests.test_image_gateway_smoke_cli -v`，确认先失败
- [ ] 实现 `run` 子命令、stdout JSON 输出、返回码逻辑
- [ ] 重新运行 `python -m unittest tests.test_image_gateway_smoke_cli -v`
...
- [ ] 运行 `python -m unittest tests.test_image_gateway_smoke tests.test_image_gateway_smoke_cli tests.test_image_provider_config tests.test_image_generation_openai -v`
- [ ] 运行 `python -m py_compile temu_y2_women\\image_gateway_smoke_http.py temu_y2_women\\image_gateway_smoke.py temu_y2_women\\image_gateway_smoke_cli.py`

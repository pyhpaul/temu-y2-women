...
_SECTION_HEADINGS = {section_id: heading for section_id, heading, _ in _SECTION_RULES}
...
def parse_whowhatwear_editorial_html(source: dict[str, Any], html: str, fetched_at: str) -> dict[str, Any]:
    ...
    sections = [_build_section_record(parser, section_id, heading, tags) for section_id, heading, tags in _SECTION_RULES]
    return {
...
class _WhoWhatWearHtmlParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "title":
            ...
            self._current_heading_section = _section_id_from_html_id(attr_map.get("id", ""))
...
def _section_id_from_html_id(value: str) -> str | None:
    if not value.startswith("section-"):
        return None
    for section_id in _SECTION_HEADINGS:
